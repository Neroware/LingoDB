    ...
    func.func @main() {
        %subop_result = subop.execution_group (){
            ...
            %loop = subop.loop %zero : !subop.simple_state<[zero : i32]> (%arg) -> !subop.simple_state<[loop : i32]> {
                ...
                %iLStream0 = graph.subop.scan_graph %graph : !graph.graph<[vx : !graph.node_set<[vx_it : !graph.graph_set_iterator<["all"]>]>],[ex : !graph.edge_set<[ex_it : !graph.graph_set_iterator<["all"]>]>]> @nodes0::@set({type = !graph.node_set<[vx_it : !graph.graph_set_iterator<["all"]>]>}), @edges0::@set({type = !graph.edge_set<[ex_it : !graph.graph_set_iterator<["all"]>]>})
                %iLStream1 = graph.node_count %iLStream0, %graph -> @graph0::@numVertices({type = index}) : !graph.graph<[vx : !graph.node_set<[vx_it : !graph.graph_set_iterator<["all"]>]>],[ex : !graph.edge_set<[ex_it : !graph.graph_set_iterator<["all"]>]>]>
            ...
            %result_stream = subop.scan_refs %loop : !subop.simple_state<[loop : i32]> @loop::@ref({type=!subop.entry_ref<!subop.simple_state<[loop : i32]>>})
            %result_graph_scan = subop.nested_map %result_stream [@loop::@ref] (%arg0, %arg1){
                %graph_stream = graph.subop.scan_graph %graph : !graph.graph<[vx : !graph.node_set<[vx_it : !graph.graph_set_iterator<["all"]>]>],[ex : !graph.edge_set<[ex_it : !graph.graph_set_iterator<["all"]>]>]> @nodes1::@set({type = !graph.node_set<[vx_it : !graph.graph_set_iterator<["all"]>]>}), @edges::@set({type = !graph.edge_set<[ex_it : !graph.graph_set_iterator<["all"]>]>})
                tuples.return %graph_stream : !tuples.tuplestream
            }
