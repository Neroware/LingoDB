...
class PropertyGraph {
    ...
    PropertyGraph(size_t maxNodeCapacity, size_t maxRelCapacity, size_t maxPropCapacity) 
        : nodes(maxNodeCapacity), relationships(maxRelCapacity), properties(maxPropCapacity), nodeSet(this), edgeSet(this), connectionsSet(this) {
            graphs.insert({(void*) nodes.ptr, this});
            graphs.insert({(void*) relationships.ptr, this});
        }
    ...
    // Runtime helper functions aiding in Property Graph iteration strategies
    ...
    static void* getEdgesLListOf(void* nodeRef);

    // Map to keep track of existing graph states
