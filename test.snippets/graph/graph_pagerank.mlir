...
module {
    func.func @main() {
    	%subop_result = subop.execution_group (){
            ...
            %streamA1 = graph.node_count %streamA0, %g -> @graph::@numVertices({type = index}) : !graph.graph<[vx : !graph.node_set<[vx_it : !graph.node_set_iterator<["all"]>]>],[ex : !graph.edge_set<[ex_it : !graph.edge_set_iterator<["all"]>]>]>
