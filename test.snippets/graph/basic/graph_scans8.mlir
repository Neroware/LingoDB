module {
    func.func @main() {
    	%subop_result = subop.execution_group (){
            ...
            %streamA4 = subop.gather %streamA3 @edges::@ref {edge_id => @edges::@id({type = i64}), to => @edges::@toRef({type = !graph.node_ref<[node_id2 : i64],[incoming2 : !graph.edge_set<[incoming_it2 : !graph.graph_set_iterator<["incoming"]>]>],[outgoing2 : !graph.edge_set<[outgoing_it2 : !graph.graph_set_iterator<["outgoing"]>]>],[property2 : i64]>})}
            ...
            %streamB4 = subop.gather %streamB3 @edges::@ref {edge_id => @edges::@id({type = i64}), to => @edges::@toRef({type = !graph.node_ref<[node_id2 : i64],[incoming2 : !graph.edge_set<[incoming_it2 : !graph.graph_set_iterator<["incoming"]>]>],[outgoing2 : !graph.edge_set<[outgoing_it2 : !graph.graph_set_iterator<["outgoing"]>]>],[property2 : i64]>}), edge_prop => @edges::@property({type = i64})}
            %streamB5 = subop.gather %streamB4 @edges::@toRef {node_id2 => @nodes::@id({type = i64}), property2 => @nodes::@property({type = i64})}

            %0 = subop.create !subop.result_table<[int64p0 : i64, int64p1 : i64, int64p2 : i64, int64p3 : i64]>
            subop.materialize %streamB5 {@edges::@id => int64p0, @edges::@property => int64p1, @nodes::@id => int64p2, @nodes::@property => int64p3}, %0 : !subop.result_table<[int64p0 : i64, int64p1 : i64, int64p2 : i64, int64p3 : i64]>
