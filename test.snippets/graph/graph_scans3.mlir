...
module {
    func.func @main() {
    	%subop_result = subop.execution_group (){
            ...
            %toNodes = subop.gather %ex @edges::@ref {to => @edges::@toRef({type = !graph.node_ref<[node_id2 : i64],[incoming2 : !graph.edge_set<[incoming_it2 : !graph.edge_set_iterator<["incoming"]>]>],[outgoing2 : !graph.edge_set<[outgoing_it2 : !graph.edge_set_iterator<["outgoing"]>]>],[property2 : i64]>})}
