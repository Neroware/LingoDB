...
module {
    func.func @main() {
    	%subop_result = subop.execution_group (){
            ...
            %toNodes = subop.gather %ex @edges::@ref {to => @edges::@toRef({type = !graph.node_ref<[node_id2 : i64],[incoming2 : !graph.edge_set<[incoming_it2 : !graph.graph_set_iterator<["incoming"]>]>],[outgoing2 : !graph.edge_set<[outgoing_it2 : !graph.graph_set_iterator<["outgoing"]>]>],[property2 : i64]>})}
            %toNodes0 = subop.gather %toNodes @edges::@toRef {node_id2 => @nodes2::@id({type = i64})}

            %0 = subop.create !subop.result_table<[int64p0 : i64]>
            subop.materialize %toNodes0 {@nodes2::@id => int64p0}, %0 : !subop.result_table<[int64p0 : i64]>
