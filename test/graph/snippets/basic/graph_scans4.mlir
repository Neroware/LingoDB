module {
    func.func @main() {
    	%subop_result = subop.execution_group (){
            ...
            %resultEdges = subop.gather %ex @edges::@ref {edge_id => @edges::@id({type = i32}), to => @edges::@toRef({type = !graph.node_ref<[node_id2 : i32],[incoming2 : !graph.edge_set<[incoming_it2 : !graph.graph_set_iterator<["incoming"]>]>],[outgoing2 : !graph.edge_set<[outgoing_it2 : !graph.graph_set_iterator<["outgoing"]>]>],[property2 : i64]>})}
