module {
    func.func @main() {
    	%subop_result = subop.execution_group (){
            %g = graph.subop.get_external_graph {name = "coffee", uniqueId = "https://github.com/Neroware/LingoDB/resources/rdf/coffee.ttl", format = TTL} : !graph.graph<[vx : !graph.node_set<[vx_it : !graph.graph_set_iterator<["all"]>]>],[ex : !graph.edge_set<[ex_it : !graph.graph_set_iterator<["all"]>]>]>
